...
*/
    ...
    {
        //$code = (int)$this->weather_forecast[$day]->attributes()->code;
        //return translateDescription($this->weather_forecast[$day]->attributes()->code);
       // $test = $this->translateDescription($code);
        
        return $this->translateDescription((int)$this->weather_condition[0]->attributes()->code);
    }
...
*/
    ...
    public function getForecastHigh($day)
    {
    	$showunit = true;
        $temperature = $this->weather_forecast[$day]->attributes()->high;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
    ...
    {
    	$showunit = true;
        $tag = $this->weather_forecast[$day]->attributes()->day;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
        
        return $this->getDay($tag);
    }      

    public function getForecastLow($day)
    {
    	$showunit = true;
        $temperature = $this->weather_forecast[$day]->attributes()->low;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
        return $temperature;
    }  
...
* @access public
*/
    public function getForecastConditionCode($day)
    {
       // global
        return (int)$this->weather_forecast[$day]->attributes()->code;
    }
...
* @access public
*/
    public function getForecastDescription($day)
    {
        $code = (int)$this->weather_forecast[$day]->attributes()->code;
        //return translateDescription($this->weather_forecast[$day]->attributes()->code);
        $test = $this->translateDescription($code);
	...
	private function translateDescription($code){
		
		if($code == 0) return 'Tornado';
		if($code == 1) return 'Tropischer Sturm';
		if($code == 2) return 'Hurricane';
		if($code == 3) return 'desc';
		if($code == 4) return 'desc';
		if($code == 5) return 'Schnee Regen';
		if($code == 6) return 'Graupel Regen';
		if($code == 7) return 'Graupel Schnee';
		if($code == 8) return 'Gefr. Niesel';
		if($code == 9) return 'Niesel Regen';
		if($code == 10) return 'Gefr. Regen';
		if($code == 11) return 'Regen';
		if($code == 12) return 'Regen';
		if($code == 13) return 'Schneetreiben';
		if($code == 14) return 'desc';
		if($code == 15) return 'desc';
		if($code == 16) return 'Schnee';
		if($code == 17) return 'Hagel';
		if($code == 18) return 'Graupel';
		if($code == 19) return 'desc';
		if($code == 20) return 'Nebel';
		if($code == 21) return 'Staubig';
		if($code == 22) return 'Rauchig';
		if($code == 23) return 'Stürmisch';
		if($code == 24) return 'Windig';
		if($code == 25) return 'Kalt';
		if($code == 26) return 'Wolkig';
		if($code == 27) return 'Meist Wolkig(Nacht)';
		if($code == 28) return 'Meist Wolkig(Tag)';
		if($code == 29) return 'Teilweise Bewölkt';
		if($code == 30) return 'Teilweise Bewölkt';
		if($code == 31) return 'Klare Nacht';
		if($code == 32) return 'Sonnig';
		if($code == 33) return 'Schön';
		if($code == 34) return 'Schön';
		if($code == 35) return 'Regen /Hagel';
		if($code == 36) return 'Heis';
		if($code == 37) return 'vereinzelt Gewitter';
		if($code == 38) return 'häufig Gewitter';
		if($code == 39) return 'häufig Gewitter';
		if($code == 40) return 'häufig Regen';
		if($code == 41) return 'starker Schnee';
		if($code == 42) return 'häufig Schnee';
		if($code == 43) return 'starker Schnee';
		if($code == 44) return 'meist Bewölkt';
		if($code == 45) return 'Gewitterregen';
		if($code == 46) return 'Schneeregen';
		if($code == 47) return 'vereinzelt Gewitterregen';
		if($code == 3200) return '';
		//return "Wolkig";
// 		if($code == 27) $desc = 'desc';
// 		if($code == 28) $desc = 'desc';
// 		if($code == 29) $desc = 'desc';
// 		if($code == 30) $desc = 'desc';
...
* @access public
*/
    public function getForecastDate($day)
    {
    	$date_format = 'd M Y';
        $date = new DateTime($this->weather_forecast[$day]->attributes()->date);
